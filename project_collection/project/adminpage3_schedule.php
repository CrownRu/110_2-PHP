...
<style type="text/css">
    ...
    <body>
        ...
        <td valign="bottom" width="200"><b><font size="5">Schedule</b></td>
        ...
        <?php
            ...
            $SQL="SELECT * FROM schedule ORDER BY schedule.time,schedule.period ASC";
            
            if($result=mysqli_query($link, $SQL)){
                echo "<table class='main'>"; 
                echo "<thead><td height='50px'>Time</td><td>period</td><td>Court</td><td>Team1</td><td>Team2</td><td></td> </thead>";
                while($row=mysqli_fetch_assoc($result)){
                    echo "<tbody>";
                    echo "<tr>";
                    echo "<td height='50px'>".$row["time"]."</td>
                    <td>".$row["period"]."</td>
                    <td>".$row["court"]."</td>
                    <td>".$row["teamOne"]."</td>
                    <td>".$row["teamTwo"]."</td>
                    <td class='main'><button><a href='deleteschedule.php?time=".$row["time"]."&period=".$row["period"]."&court=".$row["court"]."'>刪除</a></button></td>";
                    echo "</tr>";
                    echo "</tbody>";
                }
                echo "</table>";
